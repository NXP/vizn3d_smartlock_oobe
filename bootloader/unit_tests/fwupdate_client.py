...
def print_usage():
    ...
    print("")
    print("       device: The target device, the sln_local_iot or sln_vizn_iot or sln_viznas_iot board <local/vizn/viznas>")
...
if (len(sys.argv) != 5 and len(sys.argv) != 6):
    ...
    elif (FWUPDATE_ENTER_OTW_MODE == "vizn" or FWUPDATE_ENTER_OTW_MODE == "viznas") : FWUPDATE_ENTER_OTW_MODE = 1
...
def unit_test_fwupdate_send_ota_command(connection):
    print("unit_test_fwupdate_send_ota_command", flush=True)
    connection_write    = connection.sendall
    connection_read     = connection.recv
    data = {
        "messageType":2
...
def unit_test_launch_ota():
    try:
        ...
        # Send the OTA command over tcp
        unit_test_fwupdate_send_ota_command(server_socket)

    finally:
        server_socket.close

    connected = False
    # Board resets wait for it to reach bootloader
    time.sleep(5)
    while not connected:
        connected = True
        try:
