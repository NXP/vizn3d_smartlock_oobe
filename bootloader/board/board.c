...
static void BOARD_ConfigMPUNonCached(uint32_t regionStartingAddress, uint8_t regionStartingNumber, uint32_t size)
{
    ...
    if (size != 0)
    {
        /* Failed to set all the memory consider resizing */
        while (1)
        {
        }
    }
...
void BOARD_ConfigMPU(void)
{
    ...
    extern uint32_t __base_SRAM_OCRAM_CACHED;
    uint32_t ocramNonCacheStart = (uint32_t)(&__base_SRAM_OCRAM_NCACHED);
    uint32_t ocramCacheStart    = (uint32_t)(&__base_SRAM_OCRAM_CACHED);
    ...
    MPU->RBAR = ARM_MPU_RBAR(7, ocramCacheStart);
    MPU->RASR = ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 1, 1, 0, ARM_MPU_REGION_SIZE_1MB);

    /* Region 8 setting: Memory with Normal type, not shareable, non-cacheable */
    MPU->RBAR = ARM_MPU_RBAR(8, ocramNonCacheStart);
    MPU->RASR = ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 0, 0, 0, ARM_MPU_REGION_SIZE_1MB);
    ...
    BOARD_ConfigMPUNonCached(nonCacheStart, 11, size);
