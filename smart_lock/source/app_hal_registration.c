...
int APP_RegisterHalDevices(void)
{
    ...
    ret = HAL_VisionAlgoDev_H264Recording_Register(0, 0, 0);
    if (ret != 0)
    {
        LOGE("HAL_VisionAlgoDev_H264Encode_Register error %d", ret);
        return ret;
    }
    ...
    if (connectivityType == kConnectivityType_WiFi)
    {
#if ENABLE_WIFI
        HAL_WiFiAWAM510_Register();
        if (ret != 0)
        {
    ...
    else if (connectivityType == kConnectivityType_BLE)
    {
#if ENABLE_QN9090
