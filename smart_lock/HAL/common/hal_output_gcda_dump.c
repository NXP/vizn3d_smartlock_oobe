...
/*
 * @brief  GCDA Dump Tool HAL driver implementation.
 */
...
static volatile uint8_t s_has_finished = 1;
static uint8_t u8StartCapture = 0;
extern TaskHandle_t gcdaTask_handle;
            ...
            if ((1 == s_UsbDeviceCDC.attach) && (1 == s_UsbDeviceCDC.startTransactions))
            {
                ...
                if ((0 != s_UsbDeviceCDC.recvSize) && (0xFFFFFFFF != s_UsbDeviceCDC.recvSize))
                {
                    /* It may be friendly to check user input here */
                        ...
                        if (gcdaTask_handle != NULL)
                            xTaskNotifyFromISR(gcdaTask_handle, 1, eSetBits, &xHigherPriorityTaskWoken);
                }
...
int32_t gcda_dump(uint8_t *buff, unsigned int size)
{
    usb_status_t error = kStatus_USB_Error;
    if (u8StartCapture == 1)
    {
        ...
        do
        {
            error = USB_DeviceCdcAcmSend(s_UsbDeviceCDC.cdcAcmHandle, s_UsbDeviceCDC.bulkInEndpoint, buff, size);
        } while (error == kStatus_USB_Busy);

        while ((s_has_finished == false) && (s_UsbDeviceCDC.startTransactions != 0))
        {
        }
...
const static output_dev_event_handler_t gcda_dump_handler = {
    .inferenceComplete = NULL,
    .inputNotify       = NULL,
    /*
     * Device Dump will be invoked in any other places manually.
     * Output manager does not need to execute this interface.
     **/
    .dump              = NULL,
};

static hal_output_status_t gcda_dump_init(const output_dev_t *dev)
{
    hal_output_status_t error = kStatus_HAL_OutputSuccess;
    if (USB_DeviceApplicationInit())
    {
        LOGE("GCDA dump over cdc failed");
...
static hal_output_status_t gcda_dump_start(const output_dev_t *dev)
{
    hal_output_status_t error = kStatus_HAL_OutputSuccess;

    if (FWK_OutputManager_RegisterEventHandler(dev, &gcda_dump_handler) != 0)
    {
        error = kStatus_HAL_OutputError;
    }
    else
    {
        LOGD("START GCDA Dump Tool...");
        USB_DeviceIsrEnable();
        Time_Current();
        vTaskDelay(200);
...
const static output_dev_operator_t gcda_dump_ops = {
    .init   = gcda_dump_init,
    .deinit = NULL,
    .start  = gcda_dump_start,
    .stop   = NULL,
};

static output_dev_t gcda_dump_dev = {
    .name         = "dump",
...
int HAL_OutputDev_GCDA_Dump_Register()
{
    int error = 0;
    ...
    LOGD("HAL_OutputDev_GCDA_Dump_Register");
    error = FWK_OutputManager_DeviceRegister(&gcda_dump_dev);
    return error;
}
