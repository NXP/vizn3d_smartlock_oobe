...
int (*s_appCallback)(enum wifi_connection_event reason);
...
static void _wlan_event_app_callback(enum wifi_connection_event reason)
{
	if (s_appCallback != NULL)
	{
		s_appCallback(reason);
	}
}
    ...
    switch (reason)
    {
        case WLAN_REASON_SUCCESS:
            ...
            else
            {
            	...
            	_wlan_event_app_callback(WIFI_CONNECTION_CONNECTED);
            }
            break;
        case WLAN_REASON_CONNECT_FAILED:
            ...
            {
            	/* If there are no task waiting call the app layer */
            	_wlan_event_app_callback(WIFI_CONNECTION_DISCONNECTED);
            }
            break;
...
static int WLP_process_results(unsigned int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        snprintf(network_buf, sizeof(network_buf) - 1,
                 "{\r\n \"ssid\":\"%s\",\r\n \"signal\":\"%ddBm\",\r\n \"channel\":%d\r\n}",
                 scan_result.ssid, -(int)scan_result.rssi,
                 (int)scan_result.channel);
